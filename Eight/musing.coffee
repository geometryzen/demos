...
material(color: number, opacity: number = 1.0, transparent: boolean = false) {
...
tick(time: number) {
    ...
    s: any = new blade.Euclidean3(Math.cos(theta/2),0,0,0,0,0,0,0);
    rotor: any = s - B * Math.sin(theta/2);

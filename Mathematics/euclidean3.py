...
try:
    ...
    assertEqual(str(e),"TypeError: w must be a <type 'float'> or <type 'int'> or <type 'long'> on line 63")
else:
...
try:
    ...
    assertEqual(str(e),"AssertionError: Quantity is not mutable on line 79")
else:
...
# The following calculation is a bit more involved in checking the result due to rounding errors.
rotated = R * e1 * ~R
error = rotated - e2
print repr(error)
quadrance = error % error
